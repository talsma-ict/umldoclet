...
final class UmlClassDiagram extends UmlDiagram {
    ...
    @Override
    Postprocessor createPostprocessor(HtmlFile html) {
        File htmlFile = html.path.toFile();
        if (pathToCompare.equals(changeHtmlFileNameExtension(relativePath(basedir, htmlFile)))) {
            return new Postprocessor(html, this, relativePath(htmlFile, diagramFile));
        }
        return null;
    }
