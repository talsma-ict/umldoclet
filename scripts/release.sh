...
create_release() {
    ...
    local minor_version="$(minor_version_of "${release_version}")"
    ...
    if [[ $(switch_to_branch "develop-v${major_version}.${minor_version}") ]]; then
        develop_branch="develop-v${major_version}.${minor_version}"
    elif [[ $(switch_to_branch "develop-v${major_version}") ]]; then
