...
#   - If a release commit fails to deploy for a transient reason, delete the broken version from bintray and click rebuild
  ...
  ./mvnw --batch-mode -s .mvn/settings.xml -nsu -DskipTests deploy

#  ./mvnw --batch-mode -s .mvn/settings.xml -Prelease -nsu -DskipTests deploy
#  # If the deployment succeeded, sync it to Maven Central. Note: this needs to be done once per project, not module, hence -N
#  if is_release_commit; then
#    ./mvnw --batch-mode -s .mvn/settings.xml -nsu -N io.zipkin.centralsync-maven-plugin:centralsync-maven-plugin:sync
#  fi
