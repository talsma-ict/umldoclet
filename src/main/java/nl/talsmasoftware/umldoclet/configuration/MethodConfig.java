...
public interface MethodConfig {
    ...
    /// @return `true` if the method must be included in the UML diagram
    /// based on its visibility, or `false` if it must be omitted.
    boolean include(Visibility methodVisibility);

        /// Whether JavaBean property accessor methods
    /// such as `getXyz()`, `isXyz()`, `setXyz(Xyz xyz)`
