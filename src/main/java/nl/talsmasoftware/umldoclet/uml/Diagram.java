...
public abstract class Diagram extends UMLNode {
    ...
    public <IPW extends IndentingCustomWriter> IPW writeTo(IPW output) {
        ...
        IndentingCustomWriter indented = output.indent();
    ...
    protected <IPW extends IndentingCustomWriter> IPW writeCustomDirectives(List<String> customDirectives, IPW output) {
        customDirectives.forEach(directive -> output.append(directive).append(System.lineSeparator()));
    ...
    /// @param <IPW>  The type of the output object.
    /// @return The same output instance for method chaining.
    private <IPW extends IndentingCustomWriter> IPW writeFooterTo(IPW output) {
    ...
    private String writePlantumlSourceToFile() throws IOException {
        ...
        try (StringBufferingWriter writer = createBufferingPlantumlFileWriter(pumlFile)) {
            writeTo(IndentingCustomWriter.wrap(writer, config.indentation()));
    ...
    private void writeCopyrightStatement(Writer output) {
        try {
            String version = Message.DOCLET_VERSION.toString(Locale.ENGLISH);
            int year = LocalDate.now().getYear();
            String sep = System.lineSeparator();
            output.append("' Copyright to this UML and generated images belongs to the author of the corresponding Java sources.").append(sep)
                    .append(sep)
                    .append("' This UML was generated by UMLDoclet (C) Copyright 2016-" + year + " Talsma ICT.").append(sep)
                    .append("' UMLDoclet " + version + " is licensed under the Apache License, version 2.0").append(sep)
                    .append("' and contains parts of PlantUML " + Version.versionString() + " (ASL) Copyright 2009-" + year + ", Arnaud Roques.").append(sep)
                    .append(sep);
        } catch (IOException ioe) {
            throw new IllegalStateException("Error writing copyright statement: " + ioe.getMessage(), ioe);
        }
    }
}
